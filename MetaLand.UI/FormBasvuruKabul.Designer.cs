namespace MetaLand.UI
{
    partial class FormBasvuruKabul
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            button1 = new Button();
            btnReddet = new Button();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Segoe UI", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label1.Location = new Point(63, 32);
            label1.Name = "label1";
            label1.Size = new Size(64, 21);
            label1.TabIndex = 0;
            label1.Text = "Teklif : ";
            // 
            // button1
            // 
            button1.Location = new Point(22, 76);
            button1.Name = "button1";
            button1.Size = new Size(75, 23);
            button1.TabIndex = 1;
            button1.Text = "Kabul Et";
            button1.UseVisualStyleBackColor = true;
            button1.Click += btnKabul_Click;
            // 
            // btnReddet
            // 
            btnReddet.Location = new Point(132, 76);
            btnReddet.Name = "btnReddet";
            btnReddet.Size = new Size(75, 23);
            btnReddet.TabIndex = 2;
            btnReddet.Text = "Reddet";
            btnReddet.UseVisualStyleBackColor = true;
            btnReddet.Click += btnReddet_Click;
            // 
            // FormBasvuruKabul
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(234, 131);
            Controls.Add(btnReddet);
            Controls.Add(button1);
            Controls.Add(label1);
            Name = "FormBasvuruKabul";
            StartPosition = FormStartPosition.CenterScreen;
            Text = "FormBasvuruKabul";
            Load += FormBasvuruKabul_Load;
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label label1;
        private Button button1;
        private Button btnReddet;
    }
}